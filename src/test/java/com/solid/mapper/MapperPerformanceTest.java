...
public class MapperPerformanceTest {
    ...
    @Test
    public void test_map_newDto_BuilderMapper_Performance() throws Exception {
        long startTime;
        long endTime;
        long duration;
        ...
        // Copy objects using the mapper
        final Mapper builderMapper =
                                   new MapperBuilder().addMapping(new MappingBuilder().source("size")
                                                                                      .destination("length")
                                                      ...
                                                      .addMapping(new MappingBuilder().source("convertValue")
                                                                                      ...
                                                                                      .type(MappingType.BI_DIRECTIONAL)
                                                                                      .build())
                                                      .build(EntityClass.class, DtoClass.class);
        startTime = System.currentTimeMillis();
        final List<DtoClass> dtos = builderMapper.map(entities);
        endTime = System.currentTimeMillis();
        duration = (endTime - startTime);
